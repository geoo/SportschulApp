...
CreateCoursePresenter.Display {
	...
	private void addDateInpuRow(final FlexTable dateInputFieldsTable, Boolean firstField) {
		...
		if ((numRow < courseDates.size()) && !(courseDates.isEmpty())) {
			for (int i = 0; i < weekDayListBox.getItemCount(); i++) {
				if (weekDayListBox.getValue(i).equals(courseDates.get(numRow).getWeekDay())) {
					weekDayListBox.setSelectedIndex(i);
				}
			}
			for (int i = 0; i < timeListBox.getItemCount(); i++) {
				if (timeListBox.getValue(i).equals(courseDates.get(numRow).getTime())) {
					timeListBox.setSelectedIndex(i);
				}
			}
		} else {
			courseDates.add(numRow, new CourseDate(weekDayListBox.getValue(weekDayListBox.getSelectedIndex()),
	...
	private void addTariffInpuRow(final FlexTable tariffInputFieldsTable, Boolean firstField) {
		...
		if ((numRow < courseTariffs.size()) && !(courseTariffs.isEmpty())) {
			for (int i = 0; i < tariffPresenceListBox.getItemCount(); i++) {
				if (tariffPresenceListBox.getValue(i).equals(courseTariffs.get(numRow).getName())) {
					tariffPresenceListBox.setSelectedIndex(i);
				}
			}
			tariffCostsTextBox.setValue(courseTariffs.get(numRow).getCosts());
		} else {
			courseTariffs.add(numRow, new CourseTariff(tariffPresenceListBox.getValue(tariffPresenceListBox.getSelectedIndex()),
	...
	public void fillForm(Course course) {
		courseNameTextBox.setText(course.getName());
		instructorTextBox.setText(course.getInstructor());
		locationTextBox.setText(course.getLocation());
		courseDates = course.getCourseDates();
		courseTariffs = course.getCourseTariffs();
